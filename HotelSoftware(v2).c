...
void startPage()    //prints the first page, which has info about hotel
 {
   printf("\n\tNear BSF Chowk, Netaji Subhash Chandra Bose Road, Mumbai City,\n\t\t\t\t Maharashtra 400001, INDIA");
   printf("\n\n                              Ph. No.:022-28537229");
   //printf("\n\n\n                             WELCOMES YOU..............");
   printf("\n\n\n\tHotel Rosemund Inn is one of the newest Hotel in Jalandhar. The Hotel is \t\tequipped with with all the general amenities and facilities that go \t\talong with memorable stay. Set amidst beautifully landscaped gardens, \t\tit proves to be a ideal dream destination for perceptive traveller.");
...
void printAvailablity()
 {
   ...
   for(i=0;i<5;i++)
     {
       printf("\n%d\t",r_no[i]);
       printf("%s\t", r_type[i]);  //cputs(r_type[i]);
       if(i==0)
...
void checkIn()
 {
       ...
       if((year[room-1]>9999)||(month[room-1]>12)||(month[room-1]<1)||(day[room-1]<1)||(((month[room-1]==1)||(month[room-1]==3)||(month[room-1]==5)||(month[room-1]==7)||(month[room-1]==8)||(month[room-1]==10)||(month[room-1]==12))&&(day[room-1]>31))||(((month[room-1]==4)||(month[room-1]==6)||(month[room-1]==9)||(month[room-1]==11))&&(day[room-1]>30))||((month[room-1]==2)&&((year[room-1]%400==0)||((year[room-1]%4==0)&&(year[room-1]%100!=0)))&&(day[room-1]>29))||((month[room-1]==2)&&(year[room-1]%4!=0)&&(day[room-1]>28)))
  {
    ...
    {
    printf("\n... Room is allocated to %s", r_cust[room-1]);
    //cputs(r_cust[room-1]);
